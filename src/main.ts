...
export async function TiledImagesGenerator(
    ...
    async function generateTile(){
        ...
        for await (let [index, item] of images.entries()){
            const sheetItemOutputPath = `${process.env.QUEUE_PROCESS_RESIZED_FILE_PATH}/${path.basename(item)}`;
            ...
            await Sharp(item).resize(segmentWidth, segmentHeight).toFile(sheetItemOutputPath);

            if(((rowCounter+1)*tileColumnCount)-1 === index)
                rowCounter++;
        }
        ...
        await Sharp(blackTemplate).composite(generatedSheetItemPath).toFile(outputPath);
